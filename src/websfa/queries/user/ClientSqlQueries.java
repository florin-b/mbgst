...
public class ClientSqlQueries {
	...
	public static String getAdresaSediulSocial() {
		...
		sqlString.append(" from sapdev.adrc a ");
		sqlString.append(" where a.client = '900' and a.addrnumber = ");
		sqlString.append(" (select k.adrnr from sapdev.kna1 k where k.mandt = '900' and k.kunnr =?) ");
	...
	public static String getLimitaCreditClient() {
		...
		sqlString.append(" k.skfor,k.ssobl, nvl((select s2.olikw+s2.ofakw from sapdev.s067 s2 where s2.mandt='900' and ");
		sqlString.append(" s2.kkber='1000' and s2.knkli=k.kunnr),0) olikw, nvl((select sum(s1.oeikw) from sapdev.s066 s1 ");
		...
		sqlString.append(" from sapdev.knkk k, clienti c ");
